...
class Charity_Elementor_Addon_Services extends Widget_Base{
	...
	*/
	protected function render() {
	    ...
	    $upload_type = !empty($settings['upload_type']) ? sanitize_text_field($settings['upload_type']) : '';
	    $service_image = !empty($settings['service_image']['id']) ? intval($settings['service_image']['id']) : '';
	    $service_icon = !empty($settings['service_icon']) ? esc_attr($settings['service_icon']) : '';
	    $services_title = !empty($settings['services_title']) ? esc_html($settings['services_title']) : '';
	    $services_title_link = !empty($settings['services_title_link']['url']) ? esc_url($settings['services_title_link']['url']) : '';
	    ...
	    $services_title_link_attr = $services_title_link ? $services_title_link_external . ' ' . $services_title_link_nofollow : '';
	    $services_content = !empty($settings['services_content']) ? wp_kses_post($settings['services_content']) : '';
	    $need_hover = !empty($settings['need_hover']) ? sanitize_text_field($settings['need_hover']) : '';
	    $need_border = !empty($settings['need_border']) ? sanitize_text_field($settings['need_border']) : '';
	    $section_alignment = !empty($settings['section_alignment']) ? sanitize_text_field($settings['section_alignment']) : '';
	    $image_alignment = !empty($settings['image_alignment']) ? sanitize_text_field($settings['image_alignment']) : '';
	    $services_more = !empty($settings['services_more']) ? esc_html($settings['services_more']) : '';
	    ...
	    $more_link_attr = $more_link_url ? $more_link_external . ' ' . $more_link_nofollow : '';
	    $service_bg_image = !empty($settings['service_bg_image']['id']) ? intval($settings['service_bg_image']['id']) : '';

	    $hover_cls = $need_hover ? ' service-hover' : '';
	    $bg_cls = $service_bg_image ? ' with-bg' : '';
	    $salign_class = $section_alignment === 'left' ? ' service-left' : ($section_alignment === 'right' ? ' service-right' : '');
	    $img_align_class = $image_alignment === 'left' ? ' img-left' : ($image_alignment === 'right' ? ' img-right' : '');
	    $border_cls = $need_border ? ' service-border' : '';

	    $image_url = $service_image ? wp_get_attachment_url($service_image) : '';
	    $nacharity_alt = $service_image ? get_post_meta($service_image, '_wp_attachment_image_alt', true) : '';

	    $services_image = $image_url ? '<div class="nacep-image' . esc_attr($img_align_class) . '"><img src="' . esc_url($image_url) . '" alt="' . esc_attr($nacharity_alt ?: $services_title) . '"></div>' : '';
	    $services_icon = $service_icon ? '<div class="nacep-icon"><i class="' . esc_attr($service_icon) . '"></i></div>' : '';

	    $icon_main = $upload_type === 'icon' ? $services_icon : $services_image;
	    $img_class = $upload_type === 'icon' ? '' : ' have-img';

	    $title_link = $services_title_link ? '<a href="' . esc_url($services_title_link) . '" ' . esc_attr($services_title_link_attr) . '>' . esc_html($services_title) . '</a>' : esc_html($services_title);
	    $title = $services_title ? '<h3 class="service-title">' . $title_link . '</h3>' : '';
	    $content = $services_content ? '<p>' . $services_content . '</p>' : '';
	    $button = $more_link_url ? '<div class="nacep-link-wrap"><a href="' . esc_url($more_link_url) . '" ' . esc_attr($more_link_attr) . ' class="nacep-link">' . esc_html($services_more) . '</a></div>' : '';
	    ...
	    $output = '<div class="nacep-service-item' . esc_attr($style_cls . $hover_cls . $border_cls . $salign_class . $img_class . $bg_cls) . '">' . $icon_main . '<div class="service-info">' . $title . $content . $button . '</div></div>';
	    echo $output;
	}
